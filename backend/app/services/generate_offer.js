 ...
 * 
 * This module is a portable, plug-and-play generator that exports Option A (pdf-lib, fast/native).
 * 
 * Dependencies:
 *   npm install pdf-lib
 * 
 * Required Assets (in the same folder by default):
 *   - 'LETTER HEAD.pdf'
 */
...
/**
 * Helper to sanitize text for standard WinAnsi encoding.
 */
...
/**
 * Helper to wrap text to fit a specified layout width.
 */
...
/**
 * Draw text with font scaling to prevent overflow of single-line variables.
 */
...
/**
 * Draw inline bold label and regular value on a single line.
 */
...
/**
 * Helper to draw inline bold title followed immediately by regular text paragraph
 */
 ...
 * @param {string} [customLetterheadPath] - Optional path to LETTER HEAD.pdf
 * @returns {Promise<Uint8Array>} PDF bytes
 */
async function generatePDF_OptionA(data, customLetterheadPath) {
  const letterheadPath = customLetterheadPath || path.join(__dirname, 'LETTER HEAD.pdf');
  let letterheadDoc;
  let useLetterhead = false;

  if (fs.existsSync(letterheadPath)) {
    const letterheadBytes = fs.readFileSync(letterheadPath);
    letterheadDoc = await PDFDocument.load(letterheadBytes);
    useLetterhead = true;
  }
  ...
  const leftMargin = 65; // Perfectly aligned with the blue line left endpoint
  const rightMargin = 60.27; // Perfectly aligned with the blue line right endpoint
  const contentWidth = pageWidth - leftMargin - rightMargin; // 470 pt
  const topMargin = 635;
...
if (require.main === module) {
  ...
  const customLetterheadPath = args[1];
  const outputPath = args[2];
  
  generatePDF_OptionA(data, customLetterheadPath).then(bytes => {
